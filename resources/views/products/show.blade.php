...
<div class="overlay-s"></div>
        <div class="jumpo col-xs-12" style="background-image: url({{ asset('/web/images/cover.jpg')}})">
            <div class="container">
                ...
                <ul>
                    ...
                    <li>
                        <a href="{{url('category/'.$product->category_id)}}">{{$product->category->getTranslatedAttribute('name',\App::getLocale())}}</a>
                    </li>
        ...
        <main class="main-content col-xs-12">
            <div class="product-wrap col-xs-12">
                ...
                <a href="javascript:void(0)" id="fav-{{$product->id}}" class="fav-{{$product->id}} fav-pro {{$product->isFavorited() ? 'fav-active' : null  }}"  onclick="favtoggle({{$product->id }},{{Auth::user() ? Auth::user()->id : null}})">
                            ...
                            <div class="author">
                                <div class="au-img" style="background-image: url({{ url('storage/'.$product->owner->avatar) }})"></div>
                                <div class="au-data">
...
@foreach($product->options as $option)
    <tr>
      ...
      <td>{{  $option->attribute ? $option->attribute->getTranslatedAttribute('name',\App::getLocale()) : null }}</td>
      <td>{{ $option->value ? $option->value->getTranslatedAttribute('value',\App::getLocale()) : null }}</td>
                        ...
                        <div class="tab-content">
                            <div class="tab-pane fade active in" id="t1">
                                <ul>
                                    ...
                                    <li>
                                        @lang('general.from') {{$price->quantity_from}} @lang('general.to') {{$price->quantity_to}}
...
@endauth
                ...
                <a href="javascript:void(0)" id="fav-{{$product->id}}" class="fav-{{$product->id}} fav-pro {{$product->isFavorited() ? 'fav-active' : null  }}"  onclick="favtoggle({{$product->id }},{{Auth::user() ? Auth::user()->id : null}})">
                            ...
                            <div class="i-data">
                                ...
                                <p>men {{$product->getTranslatedAttribute('description',\App::getLocale())}}</p>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          <div class="rate-title">
                            ...
                            <div class="rate-form">
                                <form action="{{route('add_product_review',['type' => 'product','id' => $product_id])}}" method="POST">
                                        @csrf
