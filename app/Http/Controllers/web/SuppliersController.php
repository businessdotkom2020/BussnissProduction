 ...
 public function edit(){
    ...
    public function show($supplier_id){
         ...
         $supplier_products_categories = Category::whereHas('childs.childs.products',   function ($query)  use($supplier) {
          ...
          $supplier_services_categories = Category::whereIn('id',$supplier_categories_ids)->
          whereHas('services', function (Builder $query)  use($supplier) {
            $query->where('user_id', $supplier->id);
        })->get();

          $supplier_requests_categories = Category::whereIn('id',$supplier_categories_ids)->
          whereHas('requests', function (Builder $query)  use($supplier) {
            $query->where('user_id', $supplier->id);
        })->get();
